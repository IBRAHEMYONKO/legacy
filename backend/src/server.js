...
function sign(user) { return jwt.sign({ userId: user.id, discordId: user.discordId, admin: adminIds.has(user.discordId) }, process.env.JWT_SECRET, { expiresIn: '7d' }); }
function auth(req, res, next) {
  const value = req.headers.authorization || '';
  if (!value.startsWith('Bearer ')) return res.status(401).json({ error: 'غير مصرح' });
  try { req.auth = jwt.verify(value.slice(7), process.env.JWT_SECRET); next(); } catch { return res.status(401).json({ error: 'الجلسة منتهية' }); }
}
...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try { const me = await exchangeDiscordCode(req.query.code, process.env.DISCORD_REDIRECT_URI); const user = await upsertDiscordUser(me); res.redirect(`${webUrl}/?token=${encodeURIComponent(sign(user))}`); }
...
app.get('/api/me', auth, async (req, res) => {
  const result = await pool.query(`SELECT u.id,d.discord_id,d.username,d.global_name,d.avatar_url,p.display_name,p.bio,p.banner_url,p.level,p.experience,p.premium_until,
    COALESCE((SELECT SUM(amount) FROM point_transactions WHERE user_id=u.id),0)::bigint AS points
...
app.get('/api/inventory', auth, async (req, res) => { const result = await pool.query(`SELECT i.quantity,i.acquired_at,c.* FROM inventory_items i JOIN catalog_items c ON c.id=i.item_id WHERE i.user_id=$1 ORDER BY i.acquired_at DESC`, [req.auth.userId]); res.json(result.rows); });
app.get('/api/shop', async (_req, res) => { const result = await pool.query(`SELECT * FROM catalog_items WHERE active=true ORDER BY created_at DESC`); res.json(result.rows); });
app.get('/api/leaderboards/points', async (_req, res) => { const result = await pool.query(`SELECT d.username,d.global_name,d.avatar_url,COALESCE(SUM(t.amount),0)::bigint AS points,p.level FROM users u JOIN discord_accounts d ON d.user_id=u.id JOIN profiles p ON p.user_id=u.id LEFT JOIN point_transactions t ON t.user_id=u.id GROUP BY u.id,d.username,d.global_name,d.avatar_url,p.level ORDER BY points DESC LIMIT 50`); res.json(result.rows); });

app.get('/api/admin/users', auth, admin, async (req, res) => {
  const q = `%${String(req.query.q || '').slice(0,80)}%`;
  const result = await pool.query(`SELECT u.id,d.discord_id,d.username,d.global_name,p.display_name,p.level,COALESCE((SELECT SUM(amount) FROM point_transactions WHERE user_id=u.id),0)::bigint AS points FROM users u JOIN discord_accounts d ON d.user_id=u.id JOIN profiles p ON p.user_id=u.id WHERE d.username ILIKE $1 OR d.global_name ILIKE $1 OR d.discord_id=$2 ORDER BY u.created_at DESC LIMIT 50`, [q, String(req.query.q || '')]);
  res.json(result.rows);
});
app.post('/api/admin/points', auth, admin, async (req, res) => {
  const amount = Number(req.body.amount), target = String(req.body.userId || ''), reason = String(req.body.reason || 'تعديل إداري').slice(0,200);
...
app.get('/api/admin/audit', auth, admin, async (_req, res) => { const result = await pool.query(`SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 100`); res.json(result.rows); });

// Internal endpoints used only by the Discord bot. They are protected by a separate shared secret.
app.get('/internal/user/:discordId', internal, async (req, res) => {
  const result = await pool.query(`SELECT u.id,d.discord_id,d.username,d.global_name,d.avatar_url,p.display_name,p.bio,p.level,p.experience,p.premium_until,
    COALESCE((SELECT SUM(amount) FROM point_transactions WHERE user_id=u.id),0)::bigint AS points
    FROM users u JOIN discord_accounts d ON d.user_id=u.id JOIN profiles p ON p.user_id=u.id WHERE d.discord_id=$1`, [req.params.discordId]);
  if (!result.rowCount) return res.status(404).json({ error: 'المستخدم غير مرتبط بـ LEGACY بعد' });
  res.json(result.rows[0]);
});
app.get('/internal/shop', internal, async (_req, res) => { const result = await pool.query('SELECT * FROM catalog_items WHERE active=true ORDER BY created_at DESC'); res.json(result.rows); });
app.get('/internal/audit', internal, async (_req, res) => { const result = await pool.query('SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 100'); res.json(result.rows); });
