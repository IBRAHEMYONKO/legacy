...
async function purchaseForUser(pool, userId, itemId) {
  ...
  try {
    ...
    const balance = await c.query('SELECT COALESCE(SUM(amount),0)::bigint AS points FROM point_transactions WHERE user_id=$1', [userId]);
    if (BigInt(balance.rows[0].points) < price) throw Object.assign(new Error('رصيدك غير كافٍ'), { status: 400 });
    await c.query('INSERT INTO point_transactions(user_id,amount,reason) VALUES($1,$2,$3)', [userId, -Number(price), `شراء: ${item.rows[0].name}`]);
    await c.query('INSERT INTO inventory_items(user_id,item_id,quantity) VALUES($1,$2,1) ON CONFLICT(user_id,item_id) DO UPDATE SET quantity=inventory_items.quantity+1,acquired_at=now()', [userId, item.rows[0].id]);
    await c.query('INSERT INTO purchases(user_id,item_id,price) VALUES($1,$2,$3)', [userId, item.rows[0].id, Number(price)]);
...
function registerExtendedRoutes(app, { pool, auth, admin, internal, adminIds }) {
  ...
  app.get('/api/points/history', auth, async (req, res) => { const r = await pool.query('SELECT amount,reason,created_at FROM point_transactions WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100', [req.auth.userId]); res.json(r.rows); });
  app.put('/api/profile', auth, async (req, res) => { const displayName = String(req.body.displayName ?? '').trim().slice(0, 40); const bio = String(req.body.bio ?? '').slice(0, 500); const bannerUrl = String(req.body.bannerUrl ?? '').trim().slice(0, 1000) || null; await pool.query('UPDATE profiles SET display_name=$1,bio=$2,banner_url=$3 WHERE user_id=$4', [displayName || null, bio, bannerUrl, req.auth.userId]); res.json({ ok: true }); });
  ...
  app.post('/api/shop/:itemId/buy', auth, async (req, res) => { try { const item = await purchaseForUser(pool, req.auth.userId, req.params.itemId); res.json({ ok: true, item }); } catch (e) { res.status(e.status || 500).json({ error: e.message || 'فشل الشراء' }); } });

  app.post('/api/codes/redeem', auth, async (req, res) => {
    const code = String(req.body.code || '').trim().toUpperCase(); if (!code) return res.status(400).json({ error: 'أدخل الكود' });
    const c = await pool.connect();
    try {
      await c.query('BEGIN'); const found = await c.query('SELECT * FROM redeem_codes WHERE code=$1 FOR UPDATE', [code]);
      if (!found.rowCount) throw Object.assign(new Error('الكود غير موجود'), { status: 404 }); const item = found.rows[0];
      if (!item.active) throw Object.assign(new Error('الكود متوقف'), { status: 400 }); if (item.expires_at && new Date(item.expires_at) <= new Date()) throw Object.assign(new Error('انتهت صلاحية الكود'), { status: 400 }); if (item.max_uses !== null && item.uses >= item.max_uses) throw Object.assign(new Error('اكتملت استخدامات الكود'), { status: 400 });
      const used = await c.query('SELECT COUNT(*)::int AS count FROM code_redemptions WHERE code_id=$1 AND user_id=$2', [item.id, req.auth.userId]); if (used.rows[0].count >= item.per_user_limit) throw Object.assign(new Error('سبق واستخدمت هذا الكود'), { status: 400 });
      const rewards = normalizeRewards(item.rewards);
      for (const reward of rewards) { if (reward.type === 'points') await c.query('INSERT INTO point_transactions(user_id,amount,reason) VALUES($1,$2,$3)', [req.auth.userId, reward.amount, `كود: ${code}`]); if (reward.type === 'item') { const exists = await c.query('SELECT id FROM catalog_items WHERE id=$1 AND active=true', [reward.itemId]); if (!exists.rowCount) throw Object.assign(new Error('عنصر المكافأة غير موجود'), { status: 400 }); await c.query('INSERT INTO inventory_items(user_id,item_id,quantity) VALUES($1,$2,$3) ON CONFLICT(user_id,item_id) DO UPDATE SET quantity=inventory_items.quantity+EXCLUDED.quantity,acquired_at=now()', [req.auth.userId, reward.itemId, reward.quantity]); } if (reward.type === 'premium_days') await c.query("UPDATE profiles SET premium_until=GREATEST(COALESCE(premium_until,now()),now()) + ($1 || ' days')::interval WHERE user_id=$2", [reward.days, req.auth.userId]); }
      await c.query('INSERT INTO code_redemptions(code_id,user_id) VALUES($1,$2)', [item.id, req.auth.userId]); await c.query('UPDATE redeem_codes SET uses=uses+1 WHERE id=$1', [item.id]); await c.query('COMMIT'); res.json({ ok: true, rewards });
  ...
  app.post('/internal/admin/premium', internal, async (req, res) => { const actor = String(req.body.actorDiscordId || ''), target = String(req.body.targetDiscordId || ''), days = Number(req.body.days); if (!adminIds.has(actor)) return res.status(403).json({ error: 'الإدارة فقط' }); if (!target || !Number.isSafeInteger(days) || days < 0) return res.status(400).json({ error: 'بيانات Premium غير صحيحة' }); const [a, t] = await Promise.all([pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [actor]), pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [target])]); if (!a.rowCount || !t.rowCount) return res.status(404).json({ error: 'الحساب غير مرتبط' }); if (days === 0) await pool.query('UPDATE profiles SET premium_until=NULL WHERE user_id=$1', [t.rows[0].user_id]); else await pool.query("UPDATE profiles SET premium_until=GREATEST(COALESCE(premium_until,now()),now()) + ($1 || ' days')::interval WHERE user_id=$2", [days, t.rows[0].user_id]); await pool.query('INSERT INTO audit_logs(actor_user_id,action,target_user_id,payload) VALUES($1,$2,$3,$4)', [a.rows[0].user_id, 'premium.adjust', t.rows[0].user_id, JSON.stringify({ days, source: 'discord' })]); res.json({ ok: true }); });
  ...
  app.get('/internal/user/:discordId/cosmetics', internal, async (req, res) => { const u = await pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [req.params.discordId]); if (!u.rowCount) return res.status(404).json({ error: 'المستخدم غير مرتبط بـ LEGACY بعد' }); await syncSpecialProfile(pool, u.rows[0].user_id, req.params.discordId, adminIds); const [items,roles,selected] = await Promise.all([pool.query("SELECT c.type,c.name,c.slug,c.metadata FROM inventory_items i JOIN catalog_items c ON c.id=i.item_id WHERE i.user_id=$1 AND c.active=true", [u.rows[0].user_id]), pool.query('SELECT role FROM user_roles WHERE user_id=$1', [u.rows[0].user_id]), pool.query('SELECT c.id,c.name,c.slug,c.metadata FROM profiles p LEFT JOIN catalog_items c ON c.id=p.selected_title_id WHERE p.user_id=$1', [u.rows[0].user_id])]); res.json({ items: items.rows, roles: roles.rows.map(x => x.role), selectedTitle: selected.rows[0] || null }); });
  app.post('/internal/user/:discordId/shop/:itemId/buy', internal, async (req, res) => { const u = await pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [req.params.discordId]); if (!u.rowCount) return res.status(404).json({ error: 'المستخدم غير مرتبط بـ LEGACY بعد' }); try { const item = await purchaseForUser(pool, u.rows[0].user_id, req.params.itemId); res.json({ ok: true, item }); } catch (e) { res.status(e.status || 500).json({ error: e.message || 'فشل الشراء' }); } });
