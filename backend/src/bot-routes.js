...
function registerAdminPremium(app, pool, internal, adminIds) {
  app.post('/internal/admin/premium', internal, async (req, res) => {
    const actor = String(req.body.actorDiscordId || ''); const target = String(req.body.targetDiscordId || ''); const days = Number(req.body.days);
    ...
    const a = await pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [actor]); const t = await pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [target]);
...
function registerBotRoutes(app, { pool, internal, adminIds }) {
  app.post('/internal/shop/buy', internal, async (req, res) => {
    const discordId = String(req.body.discordId || ''); const itemId = String(req.body.itemId || '');
    const user = await pool.query('SELECT user_id FROM discord_accounts WHERE discord_id=$1', [discordId]); if (!user.rowCount) return res.status(404).json({ error: 'الحساب غير مرتبط' });
    const c = await pool.connect();
    try {
      await c.query('BEGIN'); await c.query('SELECT id FROM users WHERE id=$1 FOR UPDATE', [user.rows[0].user_id]);
      const item = await c.query('SELECT * FROM catalog_items WHERE id=$1 AND active=true FOR UPDATE', [itemId]); if (!item.rowCount) throw Object.assign(new Error('العنصر غير موجود'), { status: 404 });
      const balance = await c.query('SELECT COALESCE(SUM(amount),0)::bigint AS points FROM point_transactions WHERE user_id=$1', [user.rows[0].user_id]); const price = BigInt(item.rows[0].price); if (BigInt(balance.rows[0].points) < price) throw Object.assign(new Error('رصيدك غير كافٍ'), { status: 400 });
      ...
      await c.query('INSERT INTO purchases(user_id,item_id,price) VALUES($1,$2,$3)', [user.rows[0].user_id, itemId, Number(price)]); await c.query('COMMIT'); res.json({ ok: true, item: item.rows[0] });
    } catch (e) { await c.query('ROLLBACK'); res.status(e.status || 500).json({ error: e.message }); } finally { c.release(); }
  });
  app.post('/internal/user/redeem', internal, async (req, res) => { const discordId=String(req.body.discordId||'').trim(); const code=String(req.body.code||'').trim().toUpperCase(); if(!discordId||!code)return res.status(400).json({error:'الكود والحساب مطلوبان'}); try{res.json({ok:true,rewards:await redeemForUser(pool,discordId,code)});}catch(e){res.status(e.status||500).json({error:e.message||'فشل استرداد الكود'});} });
  app.post('/internal/user/:discordId/redeem', internal, async (req, res) => { const discordId=String(req.params.discordId||'').trim(); const code=String(req.body.code||'').trim().toUpperCase(); if(!code)return res.status(400).json({error:'أدخل الكود'}); try{res.json({ok:true,rewards:await redeemForUser(pool,discordId,code)});}catch(e){res.status(e.status||500).json({error:e.message||'فشل استرداد الكود'});} });
  registerAdminPremium(app, pool, internal, adminIds);
}
