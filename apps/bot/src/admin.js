...
async function handleAdminInteraction(interaction, { internal, admins }) {
  ...
  if (section === 'points') {
    return interaction.showModal(adminModal('legacy:admin:points', 'تعديل نقاط مستخدم', [
  ...
  if (section === 'premium') {
    return interaction.showModal(adminModal('legacy:admin:premium', 'إدارة Premium', [
  ...
  if (section === 'catalog') {
    return interaction.showModal(adminModal('legacy:admin:catalog', 'إضافة عنصر للمتجر', [
