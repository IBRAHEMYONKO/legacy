...
function Landing() { const login = `${API}/auth/discord`; return <main className="landing"><div className="landing-orbit orbit-a" /><div className="landing-orbit orbit-b" /><section className="landing-card"><div className="landing-logo">L</div><span className="eyebrow">LEGACY SOCIAL PLATFORM</span><h1>مو مجرد موقع.<br /><em>هذا عالمك.</em></h1><p>هوية Discord، مجتمع، نقاط، مقتنيات، شارات وPremium — في مساحة واحدة.</p><a className="discord-button" href={login}><span>◈</span> الدخول عبر Discord</a><div className="landing-features"><span>● هوية Discord</span><span>✦ Economy</span><span>♛ Community</span></div></section></main>; }
...
function Profile({ me, refresh, flash }) { const [bio, setBio] = useState(me.bio || ''); const [banner, setBanner] = useState(me.banner_url || ''); const [cos, setCos] = useState(null); const [saving, setSaving] = useState(false); useEffect(() => { api('/api/cosmetics').then(setCos).catch(() => {}); }, []); const save = async () => { setSaving(true); try { await api('/api/profile', { method: 'PUT', body: JSON.stringify({ bio, bannerUrl: banner }) }); await refresh(); flash('تم حفظ تخصيصات البروفايل.'); } catch (e) { flash(e.message); } finally { setSaving(false); } }; const badges = cos?.badges || []; const title = cos?.selectedTitle?.name || 'بدون لقب'; return <><Section tag="IDENTITY" title="بروفايلك" text="اسمك وصورتك من Discord ولا يمكن تعديلهما من LEGACY." /><div className="profile-grid"><Tilt className="profile-preview"><div className="profile-cover" style={banner ? { backgroundImage: `linear-gradient(180deg,transparent,#071016),url(${banner})` } : {}} /><div className="discord-lock">✓ هوية Discord</div><Avatar me={me} className="profile-avatar"/><h2>{discordName(me)}</h2><p className="username">@{me.username}</p><span className="title-chip">♛ {title}</span><p className="bio">{bio || 'أضف نبذة تعريفية من إعدادات LEGACY.'}</p><div className="badge-row">{badges.length ? badges.map(x => <span key={x.id}>{x.metadata?.icon || '🏅'} {x.name}</span>) : <span>🏅 لا توجد شارات بعد</span>}</div><div className="profile-stats"><div><b>{money(me.points)}</b><small>نقطة</small></div><div><b>Lv.{me.level || 1}</b><small>المستوى</small></div><div><b>{me.premium_until ? '✓' : '—'}</b><small>Premium</small></div></div></Tilt><Tilt className="settings-card"><div className="locked-field"><span>اسم Discord</span><b>{discordName(me)}</b><i>🔒</i></div><div className="locked-field"><span>صورة Discord</span><div className="locked-user"><Avatar me={me}/><b>مزامنة مباشرة</b></div><i>🔒</i></div><label>النبذة<textarea value={bio} onChange={e => setBio(e.target.value)} maxLength={500} rows={5} placeholder="اكتب نبذة عنك..." /></label><label>الخلفية<input value={banner} onChange={e => setBanner(e.target.value)} maxLength={1000} placeholder="رابط صورة الخلفية" /></label><button className="btn primary wide" disabled={saving} onClick={save}>{saving ? 'جاري الحفظ...' : 'حفظ التخصيص'}</button></Tilt></div></>;
}
